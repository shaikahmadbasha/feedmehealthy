 ...
 *
 */
@Component("getMenuCategoriesProcessor")
public class GetMenuCategoriesProcessor extends BaseProcessor {

	private static Logger logger = Logger.getLogger(GetMenuCategoriesProcessor.class);
	...
	public Object processRequest(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			responseMap.put(ResponseConstants.menuCategories.value(), mealDAO.getMenuCategories());

			statusCode = MealConstants.STATUS_SUCCESS;

		} catch (Exception excep) {
			logger.error("Exception while fetching menu categories", excep);
		} finally {
